...
void OLED_Write(uint8_t mode, uint8_t data) {
    if (I2C_Write(OLED_ADDRESS, mode, data) < 0) {
        fprintf(stderr, "Error: Failed to write to OLED\n");
    }
}
...
void OLED_Init(void) {
    ...
    for (size_t i = 0; i < sizeof(init_cmds); i++) {
        OLED_Write(OLED_CMD, init_cmds[i]);
    }
}
...
/**
 * @brief  Fill the entire OLED display (turn on all pixels)
 */
void OLED_Fill(void) {
    for (uint8_t page = 0; page < 8; page++) {
        OLED_Write(OLED_CMD, 0xB0 + page);
...
void OLED_ShowChar(uint8_t x, uint8_t y, char ch) {
    ...
    OLED_Write(OLED_CMD, 0x00 + (x & 0x0F));
    OLED_Write(OLED_CMD, 0x10 + ((x >> 4) & 0x0F));

    const uint8_t* font_data = Font8x8[ch - 32];
    for (int i = 0; i < 8; i++) {
        OLED_Write(OLED_DATA, font_data[i]);
    }
}
